...
public class InventoryRepository {
    ...
    private Mono<InventoryResponse> cacheAndReturn(String cacheKey, InventoryResponse response) {
        ...
        return cacheService.put(cacheKey, response, cacheProperties.getInventory().getTtl())
            .thenReturn(response);
    }

    private Mono<InventoryResponse> handleErrorWithCacheFallback(Throwable t, long sku, String cacheKey) {
        return Mono.deferContextual(ctx -> {
            ...
            return cacheService.get(cacheKey, InventoryResponse.class)
                .doOnNext(cached -> structuredLogger.logMessage(ctx, LOGGER_NAME,
                    "Using cached inventory fallback for sku: " + sku))
                .switchIfEmpty(Mono.defer(() -> {
                    // Cache miss after error = backordered (-1)
                    structuredLogger.logMessage(ctx, LOGGER_NAME,
                        "No cached inventory available, marking as backordered for sku: " + sku);

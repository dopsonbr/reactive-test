...
public class CustomerService {
                            ...
                            if (exists) {
                                return Mono.error(
                                        new DuplicateCustomerException(
                                                request.storeNumber(), request.email()));
                            }
                ...
                .doOnSuccess(
                        c ->
                                logger.logMessage(
                                        ...
                                        LOGGER_NAME,
                                        String.format(
                                                "Customer created: customerId=%s, type=%s",
                                                c.customerId(), c.type())));
    }
                            ...
                            if (customer.isB2B()
                                ...
                                return Mono.error(
                                        new BusinessRuleException(
                                                "Cannot delete B2B account with active"
                                                        + " sub-accounts"));
                            }
                ...
                .doOnSuccess(
                        v ->
                                logger.logMessage(
                                        ...
                                        LOGGER_NAME,
                                        String.format(
                                                "Customer deleted: customerId=%s", customerId)));
    }
    ...
    public Mono<Customer> createSubAccount(
        ...
        return repository
                ...
                .filter(Customer::isB2B)
                .switchIfEmpty(
                        Mono.error(new BusinessRuleException("Parent must be a B2B account")))
                .flatMap(
                        parent -> {
    ...
    private Customer buildCustomer(CreateCustomerRequest request) {
        ...
        B2BInfo b2bInfo =
                request.type() == CustomerType.BUSINESS
                        ? new B2BInfo(
                                null, request.companyInfo(), List.of(), AccountTier.STANDARD, null)
                        : null;

...
class AuditServiceTest {
  ...
  void findByEntity_normalizesLimitToDefault() {
    ...
    StepVerifier.create(
            auditService.findByEntity(EntityType.CART, "cart-123", TimeRange.unbounded(), null, 0))
        .expectNext(event)
        .verifyComplete();
  ...
  void findByEntity_preservesValidLimit() {
    ...
    StepVerifier.create(
            auditService.findByEntity(EntityType.CART, "cart-123", TimeRange.unbounded(), null, 50))
        .expectNext(event)
        .verifyComplete();
  ...
  void findByEntity_normalizesNullTimeRange() {
    ...
    verify(auditRepository)
        .findByEntity(
            eq(EntityType.CART), eq("cart-123"), timeRangeCaptor.capture(), isNull(), eq(100));
  ...
  void findByStoreAndEntityType_delegatesToRepository() {
    ...
    verify(auditRepository)
        .findByStoreAndEntityType(
            eq(100), eq(EntityType.CART), eq(timeRange), eq(AuditEventType.CART_CREATED), eq(100));
  }

...
public class PostgresCartRepository implements CartRepository {
    ...
    public PostgresCartRepository(
            CartEntityRepository entityRepository, ObjectMapper objectMapper) {
    ...
    public Mono<Void> deleteById(String cartId) {
        return Mono.fromCallable(() -> UUID.fromString(cartId))
                .flatMap(entityRepository::deleteById);
    }

    @Override
    public Mono<Boolean> exists(String cartId) {
        return Mono.fromCallable(() -> UUID.fromString(cartId))
                .flatMap(entityRepository::existsById);
    }
        ...
        return Mono.fromCallable(
                () -> {
                    CartCustomer customer =
                            deserialize(
                                    entity.customerJson(), new TypeReference<CartCustomer>() {});
                    ...
                    List<AppliedDiscount> discounts =
                            deserialize(
                                    entity.discountsJson(),
                                    new TypeReference<List<AppliedDiscount>>() {});
                    List<Fulfillment> fulfillments =
                            deserialize(
                                    entity.fulfillmentsJson(),
                                    new TypeReference<List<Fulfillment>>() {});
                    CartTotals totals =
                            deserialize(entity.totalsJson(), new TypeReference<CartTotals>() {});

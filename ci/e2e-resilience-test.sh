#!/usr/bin/env bash
#
# Run k6 resilience test against product-service
#
# This test validates the service's behavior under chaos conditions
# (simulated failures, latency, etc.)
#
# Usage: ./ci/e2e-resilience-test.sh [--ci]
#
# Options:
...
if [[ "$CI_MODE" == "true" ]]; then
    echo "==> Running k6 resilience test (CI mode)..."
    docker compose --profile chaos-product run --rm k6-product-resilience
else
    echo "==> Running k6 resilience test (local mode)..."

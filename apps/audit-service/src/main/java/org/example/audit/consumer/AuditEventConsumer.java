...
public class AuditEventConsumer {
  ...
  public void init() {
    createConsumerGroup()
        .doOnSuccess(v -> log.info("Consumer group initialized: {}", properties.consumerGroup()))
  ...
  private Flux<MapRecord<String, Object, Object>> readEvents() {
    return redisTemplate
        ...
        .read(
            ...
            StreamReadOptions.empty().count(properties.batchSize()).block(Duration.ofMillis(50)),
    ...
    return Mono.defer(
        () -> {
          try {
            ...
            return Mono.error(
                new IllegalArgumentException("Failed to parse audit event: " + e.getMessage(), e));
          }
        });
  }
  ...
  private Mono<Void> handleProcessingError(
    ...
    log.error("Failed to process audit event: eventId={}, error={}", eventId, error.getMessage());

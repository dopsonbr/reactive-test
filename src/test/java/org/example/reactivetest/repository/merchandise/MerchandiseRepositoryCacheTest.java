...
class MerchandiseRepositoryCacheTest {
    ...
    void setUp() {
        ...
        repository = new MerchandiseRepository(
            webClient,
            resilience,
            structuredLogger,
            cacheService,
    ...
    void getDescription_shouldReturnFallback_onHttpError() {
        ...
        when(requestHeadersUriSpec.uri(anyString(), any(Object[].class))).thenReturn(requestHeadersSpec);
        when(requestHeadersSpec.retrieve()).thenReturn(responseSpec);
        when(responseSpec.bodyToMono(MerchandiseResponse.class)).thenReturn(Mono.just(new MerchandiseResponse("test")));
        ...
        StepVerifier.create(repository.getDescription(sku))
            .expectNextMatches(response ->
                response.description().equals("Description unavailable"))
            .verifyComplete();
    }

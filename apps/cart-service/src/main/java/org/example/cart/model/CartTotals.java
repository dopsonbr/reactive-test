...
public record CartTotals(
  ...
  public static CartTotals calculate(
      List<CartProduct> products, List<AppliedDiscount> discounts, List<Fulfillment> fulfillments) {

    // Calculate subtotal from products
    BigDecimal subtotal =
        products.stream().map(CartProduct::lineTotal).reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    BigDecimal fulfillmentTotal =
        fulfillments.stream().map(Fulfillment::cost).reduce(BigDecimal.ZERO, BigDecimal::add);

    // Tax calculation placeholder (0 for now)
    BigDecimal taxTotal = BigDecimal.ZERO;

    // Grand total = subtotal - discounts + fulfillment + tax
    BigDecimal grandTotal = subtotal.subtract(discountTotal).add(fulfillmentTotal).add(taxTotal);

...
public class PricingService {
        ...
        return Mono.zip(
                ...
                .flatMap(
                        tuple -> {
                            ...
                            return computeOptimalPricing(
                                    request, loyalty, user, discounts, markdowns);
                        });
    }
    ...
    private Mono<PricingResult> computeOptimalPricing(
        ...
        if (shippingDiscount.compareTo(BigDecimal.ZERO) > 0) {
            appliedPromotions.add(
                    new AppliedPromotion(
                            "shipping-discount",
                            loyalty.hasBenefit(BenefitType.FREE_SHIPPING)
                                    ? "LOYALTY"
                                    : "PROMO_CODE",
                            "Free shipping",
                            shippingDiscount,
    ...
    private BigDecimal applyLoyaltyDiscount(LoyaltyInfo loyalty, BigDecimal subtotal) {
        ...
        return loyalty.getBenefit(BenefitType.PERCENTAGE_DISCOUNT)
                .map(
                        benefit ->
                                subtotal.multiply(benefit.value())
    ...
    private BigDecimal calculateMarkdownValue(Markdown markdown, BigDecimal subtotal) {
        return switch (markdown.type()) {
            case PERCENTAGE ->
                    subtotal.multiply(markdown.value())
                            .divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_UP);

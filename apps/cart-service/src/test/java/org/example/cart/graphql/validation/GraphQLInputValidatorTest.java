...
class GraphQLInputValidatorTest {
          ...
          .expectErrorMatches(
              e ->
                  e instanceof ValidationException ve
                      && ve.getErrors().stream().anyMatch(err -> err.field().equals("storeNumber")))
          .verify();
    }
  }
              ...
              e ->
                  e instanceof ValidationException ve
                      && ve.getErrors().stream()
                          .anyMatch(
                              err ->
                                  err.field().equals("cartId") && err.message().equals("Required")))
          .verify();
    }
    ...
    void shouldRejectBlankCartId() {
      StepVerifier.create(validator.validateCartId("  "))
          .expectErrorMatches(
              e ->
                  e instanceof ValidationException ve
                      && ve.getErrors().stream().anyMatch(err -> err.field().equals("cartId")))
          .verify();
    }
              ...
              e ->
                  e instanceof ValidationException ve
                      && ve.getErrors().stream()
                          .anyMatch(
                              err ->
                                  err.field().equals("cartId") && err.message().contains("UUID")))
          .verify();
    }
  }
  ...
  class AddProductValidation {
    ...
    void shouldAcceptValidInput() {
      ...
      StepVerifier.create(validator.validateAddProduct(VALID_CART_ID, input)).verifyComplete();
    }
      ...
      StepVerifier.create(validator.validateAddProduct(VALID_CART_ID, input))
          .expectErrorMatches(
              e ->
                  e instanceof ValidationException ve
                      && ve.getErrors().stream()
                          .anyMatch(err -> err.field().equals("input.quantity")))
          .verify();
    }
    ...
    void shouldRejectInvalidSku() {
      ...
      StepVerifier.create(validator.validateAddProduct(VALID_CART_ID, input))
          .expectErrorMatches(
              e ->
                  e instanceof ValidationException ve
                      && ve.getErrors().stream().anyMatch(err -> err.field().equals("input.sku")))
          .verify();
    }
  ...
  class SetCustomerValidation {
    ...
    void shouldAcceptValidInput() {
      ...
      StepVerifier.create(validator.validateSetCustomer(VALID_CART_ID, input)).verifyComplete();
    }
    ...
    void shouldRejectBlankName() {
      ...
      StepVerifier.create(validator.validateSetCustomer(VALID_CART_ID, input))
          .expectErrorMatches(
              e ->
                  e instanceof ValidationException ve
                      && ve.getErrors().stream().anyMatch(err -> err.field().equals("input.name")))
          .verify();
    }
  }
  ...
  class AddFulfillmentValidation {
    ...
    void shouldAcceptValidInput() {
      ...
      StepVerifier.create(validator.validateAddFulfillment(VALID_CART_ID, input)).verifyComplete();
    }
    ...
    void shouldRejectNullType() {
      ...
      StepVerifier.create(validator.validateAddFulfillment(VALID_CART_ID, input))
          .expectErrorMatches(
              e ->
                  e instanceof ValidationException ve
                      && ve.getErrors().stream().anyMatch(err -> err.field().equals("input.type")))
          .verify();
    }
    ...
    void shouldRejectEmptySkuList() {
      ...
      StepVerifier.create(validator.validateAddFulfillment(VALID_CART_ID, input))
          .expectErrorMatches(
              e ->
                  e instanceof ValidationException ve
                      && ve.getErrors().stream().anyMatch(err -> err.field().equals("input.skus")))
          .verify();
    }

    @Test
    void shouldValidateEachSkuInList() {
      var input = new AddFulfillmentInput(FulfillmentType.DELIVERY, List.of("123456", "invalid"));
      StepVerifier.create(validator.validateAddFulfillment(VALID_CART_ID, input))
          .expectErrorMatches(
              e ->
                  e instanceof ValidationException ve
                      && ve.getErrors().stream()
                          .anyMatch(err -> err.field().equals("input.skus[1]")))
          .verify();
    }
  }
}
